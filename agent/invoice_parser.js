...
- Only set vatRate (0.075) or whtRate (0.05 or 0.10) if the user actually mentions VAT, WHT, withholding tax, or tax. Never invent tax.
...
Business info: ${JSON.stringify(businessInfo)}`;
  ...
  try {
    const parsed = await getJSONCompletion(systemPrompt, userMessage);
    return parsed;
  } catch (err) {
